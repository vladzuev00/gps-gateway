...
public final class MangoMessageDecoder {
    ...
    public MangoMessage decode(ByteBuf byteBuf) {
        ...
        Short speed = (presenceBitMask & SPEED_PRESENT_FLAG) != 0 ? byteBuf.readShort() : null;
        Short course = (presenceBitMask & COURSE_PRESENT_FLAG) != 0 ? byteBuf.readShort() : null;
        Float altitude = (presenceBitMask & ALTITUDE_PRESENT_FLAG) != 0 ? byteBuf.readFloat() : null;
        Byte satelliteCount = (presenceBitMask & SATELLITE_COUNT_PRESENT_FLAG) != 0 ? byteBuf.readByte() : null;
